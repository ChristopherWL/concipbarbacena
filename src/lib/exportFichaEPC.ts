...
export function exportFichaEPC(
  tenant: TenantInfo,
  colaborador: ColaboradorInfo,
  epcAssignments: EPCAssignment[]
) {
  ...
  // Row 2: Declaration
  const declaration = "Declaro que recebi os Equipamentos de Proteção Coletiva (EPC) relacionados abaixo e assumo integral responsabilidade sobre os mesmos, comprometendo-me a utilizá-los corretamente e zelar pela sua conservação.";
  wsData.push([
    { v: '', s: cellStyle },
    { v: '', s: cellStyle },
  ...
  // Row 4: Title
  wsData.push([
    { v: 'FICHA DE CONTROLE INDIVIDUAL DE EPC', s: { ...headerStyle, font: { bold: true, sz: 10 } } },
    { v: '', s: headerStyle },
    { v: '', s: headerStyle },
    { v: '', s: headerStyle },
    { v: '', s: headerStyle },
  ...
  wsData.push([
    ...
    { v: 'Descrição do EPC', s: headerStyle },
  ...
  // Data rows
  const minRows = 15;
  const totalRows = Math.max(minRows, epcAssignments.length);
  
  for (let i = 0; i < totalRows; i++) {
    const item = epcAssignments[i];
    if (item) {
      wsData.push([
        { v: item.quantity, s: cellCenter },
        { v: item.description, s: cellStyle },
        { v: item.location || '', s: cellCenter },
  ...
  XLSX.utils.book_append_sheet(wb, ws, 'Ficha EPC');

  const fileName = `Ficha_EPC_${colaborador.name?.replace(/\s+/g, '_') || 'Colaborador'}_${format(new Date(), 'yyyy-MM-dd')}.xlsx`;
