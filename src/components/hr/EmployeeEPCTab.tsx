...
import { useEmployeeEPC } from '@/hooks/useEmployeeEPC';
import { useProducts } from '@/hooks/useProducts';
import { useAuth } from '@/hooks/useAuth';
import { supabase } from '@/integrations/supabase/client';
import { toast } from 'sonner';
import { exportFichaEPC } from '@/lib/exportFichaEPC';
...
export function EmployeeEPCTab({
  employeeId,
  employeeName,
  employeePosition,
  employeeHireDate,
  ...
  const { assignments, isLoading, createAssignment, updateAssignment, deleteAssignment } = useEmployeeEPC(employeeId);
  ...
  const handleProductSelect = (productId: string) => {
    const product = epcProducts.find(p => p.id === productId);
    if (product) {
      setNewItem({
        ...newItem,
  ...
  const handleAdd = async () => {
    ...
    if (newItem.product_id) {
      const selectedProduct = epcProducts.find(p => p.id === newItem.product_id);
      if (!selectedProduct) {
    ...
    <>
      <Card>
        <CardHeader className="pb-2 flex flex-row items-center justify-between">
          <CardTitle className="text-sm font-medium">Ficha de Controle de EPC</CardTitle>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              EPC: <span className="font-medium text-foreground">{returnItem?.description}</span>
            </p>
            <div>
